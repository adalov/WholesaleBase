...
namespace WholesaleBase.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Edit(EditAccountViewModel model)
        {
            ...
            {
                Warehouse warehouse = db.Warehouses.Find(model.WarehouseId);
                warehouse.Name = model.Name;
                warehouse.Representative = model.Representative;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            {               
                ...
                if (result.Succeeded)
                {
                    Warehouse warehouse = new Warehouse() { Name = model.Name, Representative = model.Representative, Address = model.Address, PhoneNumber = model.PhoneNumber, Status = true };
                    db.Warehouses.Add(warehouse);
                    db.SaveChanges();

                    foreach (var goods in db.Goods.Where(g => g.Status == true).ToList())
                    {
                    ...
                    var identityClaim = new IdentityUserClaim { ClaimType = "WarehouseId", ClaimValue = warehouse.Id.ToString() };
                    user.Claims.Add(identityClaim);
                    await UserManager.UpdateAsync(user);
                    await UserManager.AddToRoleAsync(user.Id, "storekeeper");
                    return RedirectToAction("Index", "Warehouses");
                }
                AddErrors(result);
            }            
